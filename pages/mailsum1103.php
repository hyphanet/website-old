...
<p>
Some pointed out that a 'special request' could be used to determine that the node had the data, and was trying to conceil it. Newsbyte suggested that we could use the estimators for calculating the delay it would normally take, if  the node had to retrieve it from another node. Tom Kaitchuck and Martin Stone Davis had some objections to this, but seem to have thought that it was meant to be used for every node and/or with fixed time delays. However, Newsbyte pointed out that it should only be the node which has the data in it's store who would have to fake it, and that the estimator isn't fixed. Therefor, a request would only take on 'hop' longer, as Toad already indicated in his post. Toad, however, pointed out that the difficulty were the new nodes: since they have totally unrealistic estimators, it would be difficult to implement a time delay based on the estimators from the new nodes. Ofcourse, brandnew nodes would not have any data, but still, it could pose a problem. Luckely, he had some ideas to redress the problem, for instance, new node problem would be solved with estimators in the seednodes.
