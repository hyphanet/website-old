	...
	<p>En particular, usaremos esos fondos para completar Freenet 0.8, la cual será liberada 
	...
	comunicarse sobre Freenet, incluyendo "Freetalk",un sistema de foros de discusión empleando 
	una nueva aproximación a la  "web de confianza" para prevenir el spam en una forma enteramente descentralizada.</p>

	<p>En otras noticias, el Summer of Code fue un éxito calificado, con dos de nuestros estudiantes 
	desempeñandose muy bien. El soporte para ogg vorbis/theora será incorporado pronto, y la nueva 
	capa de transporte mas eficiente será incorporada un poco más tarde.</p>
	...
	código de administración de carga para un mejor y mas eficaz funcionamiento.</p>
	
	<a name="sorra-aguan-1253"><h3>14 de Junio, 2010 - Fallas serias, por favor actualiza inmediatamente</h3></a>
	<p>La versión 1253 de Freenet repara una falla muy seria, y todos los usuarios deberían actualizarse inmediatamente, especialmente aquellos 
	corriendo sistemas de 32-bit x86. Esta versión es "obligatoria", significa que no conectará a nada anterior a  1252.
	El error estaba en la aceleración nativa FEC - esta es una librería nativa (C, no Java) usada para acelerar 
	...
	sabemos si esto puede tornarse en algo mas serio, así que hemos deshabilitado la aceleración FEC 
	nativa por ahora; Freenet será levemente más lenta cuando decodifica archivos descargados, pero no  
	...
	luego agrupados en "segmentos" de hasta alrededor de 4MB. Hasta 1251, todo excepto los últimos segmento podían ser exactamente
	de 4MB, entonces el último segmento podía ser mucho mas pequeño que esto - en algunos casos teniendo sólo un block, o sólo
	unos pocos bloques. Esto podía causar que alguna descarga se atascase con sólo unos pocos bloques faltantes. Ahora todos 
	...
	asegurar que no pueden revelar tu dirección IP. Uno de nuestros estudiantes en el  Summer of Code (Spencer) ha estado
	...
	Debería ser más rápido y más fácil para el propietario del índice (la persona que corre Spider) y para los usuarios el buscar
	contenidos.</p>
	...
	<li><b>Búsqueda distribuida</b> - Tatyana Pyatigorskaya estará trabajando en búsqueda distribuída e 
	...
	ha estado fuera de línea por un período extenso (horas). Déjanos saber como va! La información es siempre bienvenida, particularmente
...
<li>Tenemos alrededor de 4500-7000 nodos en línea en cualquiera instante dado.</li>
<li>Hace 5 días, teníamos alrededor de 14000 nodos no trasientes.</li>
<li>De los nodos en línea en todo momento, bastamente 37% son nodos 24x7  (96% promedio de uptime), 33% son usuarios regulares (56% promedio de uptime), y 30% son nodos ocasionales o novatos
...
el cual tristemente requerirá el vaciado de la existente web de confianza de pruebas, así que tristemente todas las identidades y mensajes desaparecerán. Freetalk necesita más trabajo, alrededor del  50% de los errores marcados en 0.1 en el 
...
Soy consciente que se ha prometido antes, pero es importante, así que sucederá tarde o temprano, espero que temprano.</p>
<p>Una búsqueda completa distribuída basada en Web de Confianza , focalizada en compartir archivos, está lejana en el horizonte por el momento. infinity0 deberá ser capaz de hacer algún trabajo en ella como parte de sus estudios, veremos. 
...
<li>Auto-configuración de límites de memoria en los instaladores, y preguntarle al usuario sobre cuanta memoria usar (al menos en algunos casos) es relativamente fácil e importante, pero no vital.</li>
...
<p>Por favor prueba Freenet, y reporta cualquier error y problemas de usabilidad que encuentres en el  <a href="https://bugs.freenetproject.org/">rastreador de erroresr</a> o vía Freetalk tablón en.freenet (observa que este 
...
mejorar la persistencia de datos mejorará la usabilidad y la percepción del desempeño de Freenet  inmensamente. La continuada popularidad de insertar por demanda en uservoice lo demuestra mas que cualquier cosa: 
...
<p>Evan está convencido que mayormente la pobre persistencia de datos no es debida a que los datos desaparezcan de los almacenes, sino debida a que el pequeño número de nodos que almacenan los datos (como opuesto a almacenarlos en caché)  quedan fuera de línea 
o se tornan inencontrables. Hemos incrementado el número de nodos que almacenan datos, hemos hecho que el nodo use el almacén como caché si hay espacio libre, hemos hecho varias cosas apuntando 
a mejorar la persistencia de datos, y hay mucho más que podemos hacer. Una cuestión inmediata es si las mejoras de seguridad obtenidas el último año por no hacer caché en altos HTL han roto muchas 
inserciones haciendo que no se almacenen en caché en los nodos correctos; vamos a probar esto en 1241. Una cuestión relacionada es por qué insertar la misma clave 3 veces da tan enorme ganancia en el desempeño con relación a  
...
con triple-inserción de un solo bloque, y esto podría ser bien posible con algunos cambios a cómo se inserta...</p>
...
<p>El amigo de Ian pupok está trabajando en borrador de una nueva interfaz de usuario AJAXy para Freenet. la rama de web-pushing de sashee, la cual hace la interfaz de usuario mucho más dinámica sin hacerla parecer  
muy diferente, podría ser incorporada pronto, pero desactivada por defecto, ya que tiene algunos errores molestos. Cuando está activadda, resuelve el problema de las conexiones paralelas anticuadas, mostrando progresos individuales 
...
tenga o cualquier comentario que haga sobre cómo podria ser mejor.</p>
	...
	notificados por Java cuando haya actualizaciones disponibles; la situación puede ser un poco más complicada para usuarios de  
	...
	El problema es una vulnerabilidad de ejecución remota de código que afecta a Java y también algunos
	...
	<p>
	La versión 1226 está ahora disponible, incluyendo muchos cambios. Los nodos Freenet existentes la  
	...
	almacén de datos. Esto está reparado ahora. Freenet no almacenada que hayas descargado,
	...
	habilitarán nuevas cosas las cuales deberían mejorar el desempeño grandemente, también como
	...
	<li><b>Cifrado para datos locales sensibles</b><br>
	El nuevo caché de cliente y la base de datos de descargas/subidas pueden ahora ser cifradas.
	Esto es controlado por la configuración del "nivel de seguridad física". Si lo estableces en
	BAJO, nada es cifrado, ni siquiera los archivos temporales, y Freenet usará menos
	CPU y trabajará levemente más rapido. Si lo estableces en NORMAL o más alto, Freenet 
	ahora cifrará tu caché de cliente y base de datos de descargas. En NORMAL, las claves son 
	guardadas en u archivo master.keys, así si borras este archivo seguramente (o haces click en el
	...
	haciendo con él. Así tienes un control detallado sobre el balance entre seguridad,
	...
	la búsqueda y transferencia de datos a través de muchos otros nodos. Esto debería 
	...
	demanda, haciendo muy difícil para un adversario remover
	contenidos.  Freenet automáticamente creará más copias del
	contenido popular para asegurar que siempre estará disponible.
      </p>
      <p>
	Freenet 0.7 introduce el concepto "darknet", permitiendo a los usuarios
	conectar solamente con sus amigos confiables (y a través de ellos
	...
	<li>
	  ...
	  reinicio. Esto incrementa la velocidad de Freenet y reduce su
	...
	<li>
	  ...
	  archivo tomará sólo unos pocos segundos, integra la búsqueda en la
	  página de navegación, y generalmente mejora la usabilidad en 
	  muchas áreas.
	</li>
	<li>
	  Desempeño significativamente mejorado para insertar y
	  recuperar archivos y especialmente páginas, y también para
	  la conexión inicial de Freenet a la red.
	</li>
	<li>
	  Un nuevo instalador para Windows el cual trabaja con Vista tan bien
	...
	a nuestra <a class="maybeSwitchProtocol" href="https://emu.freenetproject.org/cgi-bin/mailman/listinfo/devl/">lista de correo de
	...
	un nuevo instalador Vista-compatible para Windows (parte estará listo en 
